...
async function convertProjectToSingleHost(host) {
  ...
  const srcContent = await readFileAsync(`./src/taskpane/app/${host}.app.component.js`, "utf8");
  await writeFileAsync(`./src/taskpane/app/app.component.js`, srcContent);
  ...
  if (convertTest && (host === "excel" || host === "word")) {
    ...
    const updatedTestTaskpaneContent = testTaskpaneContent.replace(`../../src/taskpane/app/${host}.app.component`, `../../src/taskpane/app/app.component.js`);
  ...
  hosts.forEach(async function (host) {
    ...
    await unlinkFileAsync(`./src/taskpane/app/${host}.app.component.js`);
  });
